...
const FadeInText = ({ lines1 = defaultLines1, lines2 = defaultLines2 }: FadeInTextProps) => {
    const containerRef = useRef<HTMLDivElement>(null);
    const section1Ref = useRef<HTMLDivElement>(null);
    const section2Ref = useRef<HTMLDivElement>(null);

    useGSAP(() => {
        const lineElements1 = gsap.utils.toArray(".line-1");
        const lineElements2 = gsap.utils.toArray(".line-2");
    ...
    return (
        <div className='h-screen w-full relative bg-[#F3EAD8] overflow-hidden z-10' ref={containerRef}>
            ...
            >
                {lines1.map((line, i) => (
                    <h2 key={`line1-${i}`} className='line-1 mt-5 md:mt-2'>{line}</h2>
                ))}
            </div>
            ...
            >
                {lines2.map((line, i) => (
                    <h2 key={`line2-${i}`} className='line-2 mt-5 md:mt-2'>{line}</h2>
                ))}
            </div>
        </div>
    );
